...
const sortDataOnRating = (fileContents, cb) => {
	...
	const sortedData = fileContents.filter((a) => !isNaN(+a.product_rating)).sort((a, b) => b.product_rating - a.product_rating)
...
const writeSortedDataToFile = (outputFileName, fileName) => {
	...
	readFileContents(fileName, (err, data) => {
		if (err) {
			...
			fileContentsWithRating = data.map((stringData) => stringData.replace(' ', '').split(',')).filter((data) => !isNaN(+data[data.length - 1]))
			fileContentsWithRating.forEach((data) => data.push('\n'))
